...
module.exports = {
	...
	getProductBySlug: async (req, res) => {
		try {
			...
			if (product) {
				...
				res.status(400).send(
					`Cannot find the slug "${req.params.slug}"`
				);
		} catch (e) {
	...
	getProductById: async (req, res) => {
		try {
			...
			if (product) {
				...
				res.status(400).send(
					`Cannot find product with id "${req.params.id}"`
				);
		} catch (e) {
			res.status(400).send("Error to listing the product " + e);
		}
	...
	getProductsByTag: async (req, res) => {
		try {
			...
			if (products.length > 0) {
				...
				res.status(400).send(
					`Cannot find products with tag "${req.params.tag}"`
				);
		} catch (e) {
			res.status(400).send("Error to listing the product " + e);
		}
	...
	addProduct: async (req, res) => {
		...
		form.parse(req, async (err, fields, files) => {
			...
			contract.hasMinLen(
				fields.title,
				3,
				"the title must be have at least 3 caracters"
			);
			...
			try {
				...
				res.status(201).send(
					`The "${fields.title}" has been registred.`
				);
			} catch (e) {
	...
	removeProduct: async (req, res) => {
		try {
			...
			res.status(200).send(
				`The product "${product.title}" has been deleted.`
			);
		} catch (e) {
			res.status(400).send("An error ocurred " + e);
		}
